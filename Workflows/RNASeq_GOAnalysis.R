...
#fisher.mode="classic"
fisher.mode="partentChild"
########################GO-Analysis
setwd(dir = "/Work2/Tim_D_Sequencing/2021_11_25_Epidermal_Immuno_EcoliPolyIC/08.2-DE/RNASeq/04-GOAnalysis")
dir.create("./GO_out_graphs")
dir.create("./GO_out_graphs/GO_Networks")
dir.create("./GO_out_graphs/barplot_out")
...
geneID2GO <- read.csv('/Work2/Planarian_genome_2021/SMEST2G_2GO/smes_v2_hconf_only1SMEST_toGO.tsv', header=F, sep = '\t', stringsAsFactors = T) #geneID2GO <- geneID2GO[,c(3,5)]
...
for (inputfile in all_files) { 
  ...
  ################GO-Plots
  #Top 30 Genes (highest Percentage of associated Genes present; filter:  Annotated-No. = >20; p-value?, Expected-No.?,)
  ...
  head(GOStatPlot)
  head(Plotting)
  write.table(Plotting, file= paste(tsv_path,"Plotted_Terms/",inputfile,".",fisher.mode,".plotted.tsv", sep=""), col.names = NA, row.names = T, quote = F, sep='\t')
  pdf(file = paste(output_path,"barplot_out/",inputfile,".",fisher.mode,".top35.pdf",sep=""), width = 10, height = 10, )
  par(mar=c(5,32,21,5))
  ...
  pdf(file = paste(output_path,"barplot_out/",inputfile,".",fisher.mode,".top35_percentage.pdf",sep=""), width = 10, height = 10, )
  par(mar=c(5,32,21,5))
  barplot(Plotting$Percentage,
          names.arg = paste(row.names(Plotting),Plotting$`GO-Term`, sep=" : "), horiz = TRUE, #über col könnte noch eine Farbcodierung eingefügt werden
          xlab="Percentage associated Genes", cex.names = 0.7, 
